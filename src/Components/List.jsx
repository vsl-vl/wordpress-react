...
export default function List () {
    ...
    const { id } = useParams();
    const catSlug = id == null ? `wp/v2/posts?per_page=${page}` : `wp/v2/posts?per_page=${page}&categories=${id}`
        ...
        <div>
            ...
            <div>
                {posts == false ? "Yüklənir..." : (
                    posts.map((post) => 
                        <div onClick={() => {setPos(true)}} key={ post.id } className="border-[1px] border-[rgba(0,0,0,0.2)] py-4 px-6 my-2 rounded-md flex flex-col gap-2 shadow-3xl">
